...
//验证用户上传文件
if ($('#editor').length) {//editor验证id为editor的元素，即user文件夹下的add.html文件内,
    ClassicEditor
        .create(document.querySelector('#editor'), {
            language: 'zh-cn',
            ckfinder: {
                uploadUrl: '/user/article/ckeditor'  //因为app主路由处已经改成/user/*，所以这里写什么也无所谓
            }
        })
        .catch(error => {
            console.error(error);
...
function showToasts(title, message) {
    if ($('.toasts').length === 0) {
        $('body').append($('<div class="toasts" style="z-index: 20; position: fixed; right: 10px; top: 0"></div>'))
    }
